...
namespace FHS.CT.AlgoDat.DataStructures
{
    public class BinarySearchTree<T> where T : IComparable<T>
    {
        ...
        public Node? FindKthMinimum(int k)
        {
            if (k < 0)
            {
                throw new ArgumentException("k must be larger than 0");
            }
            ...
            Node? currentMinimum = Minimum(Root);
            for (int i = 1; i < k; i++)
            {
                ...
                currentMinimum = Successor(currentMinimum);
            }
