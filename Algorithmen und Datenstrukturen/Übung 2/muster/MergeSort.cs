...
namespace AlgoDat
{
    ...
    {
        public static void Sort(T[] list, int cutOff)
        {
            Sort(list, 0, list.Length - 1, cutOff);
        }

        private static void Sort(T[] list, int left, int right, int cutOff)
        {
            if (right - left + 1 <= cutOff)
            {
                InsertionSort<T>.Sort(list, left, right);
            }
            else if (left < right)
            {
                int middle = (left + right) / 2;
                Sort(list, left, middle, cutOff);
                Sort(list, middle + 1, right, cutOff);
